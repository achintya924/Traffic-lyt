...
def get_hotspot_grid(
    ...
    anchor_end = filters.end or now; recent = [anchor_end - recent_days, anchor_end];
    baseline = [baseline_end - baseline_days, baseline_end] with baseline_end = recent_start.
    Clamps to filters.start when set.
    """
    grid_size_deg = cell_m / METERS_PER_DEGREE_APPROX
    anchor_end = filters.end if filters.end is not None else datetime.utcnow()
